...
module.exports = function (options) {
  ...
  function getObject(req, res, next, isHEADRequest) {
    ...
    if (req.header('Range') && compatibleRangeHeader.test(req.header('Range').trim())) {
      s3Params.Range = req.header('Range');
    }
    ...
    if (isHEADRequest) {
      s3Request = s3.headObject(s3Params);
    } else {
  ...
  return function (req, res, next) {
    ...
    if (!options.defaultKey && req.path.slice(-1) === '/') {
      ...
      getObject(req, res, next, req.method === 'HEAD');
    }
  };
};
